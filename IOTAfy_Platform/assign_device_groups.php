...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['assign_group'])) {
        ...
        try {
            ...
            $stmt = $db->prepare("SELECT COUNT(*) FROM device_groups WHERE device_id = :device_id AND group_id = :group_id");
            $stmt->execute(['device_id' => $device_id, 'group_id' => $group_id]);
            if ($stmt->fetchColumn() > 0) {
                $message = "Device is already assigned to this group.";
            } else {
                $stmt = $db->prepare("INSERT INTO device_groups (device_id, group_id, assigned_date) VALUES (:device_id, :group_id, datetime('now'))");
                $stmt->execute(['device_id' => $device_id, 'group_id' => $group_id]);
            ...
            $message = "Error assigning device to group: " . $e->getMessage();
            logMessage("Error assigning device to group: " . $e->getMessage() . " by user {$_SESSION['user_id']}.");
        }
        ...
        try {
            ...
            $message = "Error unassigning device from group: " . $e->getMessage();
            logMessage("Error unassigning device from group: " . $e->getMessage() . " by user {$_SESSION['user_id']}.");
        }
    }
}
...
try {
    ...
    $message = "Error fetching device details: " . $e->getMessage();
    logMessage("Error fetching device details: " . $e->getMessage() . " by user {$_SESSION['user_id']}.");
    $device = null;
}
...
<head>
    ...
    <title>Assign Groups to Device</title>
...
<body>
    <div class="container mt-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1>Assign Groups to Device</h1>
            <a href="device_management.php" class="btn btn-secondary">
