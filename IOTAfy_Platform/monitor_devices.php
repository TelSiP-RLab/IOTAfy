...
function sendNotification($deviceRow, $status) {
    ...
    if (in_array($preference, ['email', 'both'], true) && filter_var($userEmail, FILTER_VALIDATE_EMAIL)) {
        ...
        $mailResult = @mail($userEmail, EMAIL_SUBJECT, $message, $headersStr);
        if ($mailResult) {
            ...
            logMessage("Email notification FAILED to {$userEmail} for Device ID {$deviceId}: {$errorMsg}");
            monitorLog("Notification FAILED: type=email, user_id={$userId}, email={$userEmail}, device_id={$deviceId}, error={$errorMsg}");
        }
    }
