...
# Only configure msmtp in monitor container, not in platform container
if [ "$CONTAINER_TYPE" = "monitor" ] && [ -n "$SMTP_HOST" ]; then
...
# Note: msmtp configuration is only done in monitor container, not in platform container
